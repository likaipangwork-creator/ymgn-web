import { useMemo, useState } from 'react'
import { useNavigate, useParams } from 'react-router-dom'
import { PageToolbar } from '../../components/PageToolbar'
import { useAuth } from '../../context/AuthContext'
import { useData } from '../../context/DataContext'
import type { OAMaintenanceEquipmentItem, OAReportCategory, OAProcurementSubType } from '../../types/models'

export function EditOAPage() {
  const { id } = useParams<{ id: string }>()
  const navigate = useNavigate()
  const { username, isSuperAdmin } = useAuth()
  const { oaReports, equipments, updateOAReport, rentableStock } = useData()
  const existing = oaReports.find((r) => r.id === id)

  const [category, setCategory] = useState<OAReportCategory>(existing?.category ?? '维修')
  const [procurementSubType, setProcurementSubType] = useState<OAProcurementSubType>(
    existing?.procurementSubType ?? '愿望单'
  )
  const [title, setTitle] = useState(existing?.title ?? '')
  const [detail, setDetail] = useState(existing?.detail ?? '')
  const [executorUsername, setExecutorUsername] = useState(existing?.executorUsername ?? username)
  const [maintenanceItems, setMaintenanceItems] = useState<OAMaintenanceEquipmentItem[]>(
    existing?.maintenanceEquipmentItems ?? []
  )
  const [error, setError] = useState<string | null>(null)
  const [saving, setSaving] = useState(false)

  const reportId = existing?.id ?? id ?? ''

  const maintenanceEquipments = useMemo(
    () =>
      equipments.filter(
        (eq) =>
          rentableStock(eq, reportId) > 0 || maintenanceItems.some((i) => i.equipmentId === eq.id)
      ),
    [equipments, rentableStock, maintenanceItems, reportId]
  )

  if (!existing) {
    return (
      <div className="stack">
        <PageToolbar backTo="/oa" title="编辑提报" />
        <p className="muted page-center">提报不存在</p>
      </div>
    )
  }

  if (!isSuperAdmin) {
    return (
      <div className="stack">
        <PageToolbar backTo={`/oa/${id}`} title="编辑提报" />
        <p className="muted page-center">仅超级管理员可编辑</p>
      </div>
    )
  }

  const getQty = (equipmentId: string) =>
    maintenanceItems.find((i) => i.equipmentId === equipmentId)?.quantity ?? 0

  const setQty = (equipmentId: string, quantity: number) => {
    setMaintenanceItems((prev) => {
      const rest = prev.filter((i) => i.equipmentId !== equipmentId)
      if (quantity <= 0) return rest
      return [...rest, { equipmentId, quantity }]
    })
  }

  const handleSave = async () => {
    setSaving(true)
    setError(null)
    try {
      const err = await updateOAReport({
        id: existing.id,
        editorUsername: username,
        category,
        procurementSubType: category === '采购' ? procurementSubType : null,
        title,
        detail,
        executorUsername,
        maintenanceEquipmentItems: maintenanceItems,
      })
      if (err) {
        setError(err)
        return
      }
      navigate(`/oa/${id}`)
    } finally {
      setSaving(false)
    }
  }

  return (
    <div className="stack">
      <PageToolbar backTo={`/oa/${id}`} title="编辑提报" />

      <section className="panel form-stack">
        <label>
          类别
          <select className="form-input" value={category} onChange={(e) => setCategory(e.target.value as OAReportCategory)}>
            <option value="维修">维修</option>
            <option value="采购">采购</option>
          </select>
        </label>

        {category === '采购' ? (
          <label>
            采购类型
            <select
              className="form-input"
              value={procurementSubType ?? '愿望单'}
              onChange={(e) => setProcurementSubType(e.target.value as OAProcurementSubType)}
            >
              <option value="愿望单">愿望单</option>
              <option value="耗材采购">耗材采购</option>
            </select>
          </label>
        ) : null}

        <label>
          标题
          <input className="form-input" value={title} onChange={(e) => setTitle(e.target.value)} />
        </label>
        <label>
          申报内容
          <textarea className="form-input" rows={4} value={detail} onChange={(e) => setDetail(e.target.value)} />
        </label>
        <label>
          执行人
          <input className="form-input" value={executorUsername} onChange={(e) => setExecutorUsername(e.target.value)} />
        </label>

        {category === '维修' ? (
          <div>
            <h3>维护器材</h3>
            <div className="card-list">
              {maintenanceEquipments.map((eq) => {
                const qty = getQty(eq.id)
                const max = rentableStock(eq, reportId) + qty
                return (
                  <div key={eq.id} className="equipment-picker-row">
                    <div>
                      <p>{eq.name}</p>
                      <p className="muted">可送修 {max} 件</p>
                    </div>
                    <div className="qty-control">
                      <button type="button" className="qty-btn" disabled={qty <= 0} onClick={() => setQty(eq.id, qty - 1)}>
                        −
                      </button>
                      <span className="qty-value">{qty}</span>
                      <button type="button" className="qty-btn" disabled={qty >= max} onClick={() => setQty(eq.id, qty + 1)}>
                        +
                      </button>
                    </div>
                  </div>
                )
              })}
            </div>
          </div>
        ) : null}

        {error ? <p className="error-text">{error}</p> : null}
        <button type="button" className="btn btn-primary" disabled={saving} onClick={() => void handleSave()}>
          {saving ? '保存中…' : '保存'}
        </button>
      </section>
    </div>
  )
}
